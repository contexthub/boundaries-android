...
2. Use the ContextHub.addContextEventListener(ContextEventListener listener) method to hook into the events generated by ContextHub.
...
In addition, `GeofencesMapFragment` responds to any events created from the sensor pipeline through the `SensorPipelineListener.onEventPosted` notification. At that point, you'll be able to filter whether the event was a geofence event which you were interested in and respond accordingly. There are several pre-defined keys that let you access information stored in an event, such as event name, state, type, etc..
